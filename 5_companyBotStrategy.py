...
and 0 if no answer was given. In this case, the bot's correct answer time 
for a given task would be the average of the answer times from the trainers who answered correctly. 
...
def solution(trainingData):
    ...
    for i in range(len(trainingData)):
        if trainingData[i][1] == 1:
            answerTime.append(trainingData[i][0])
    if len(answerTime) != 0:
        return sum(answerTime) / len(answerTime)
    return 0
